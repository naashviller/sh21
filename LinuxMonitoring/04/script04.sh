...
if [ -z "$clr_1" ]
then
    echo "Column 1 background = default ($(outputcolor 3))"
else
    echo "Column 1 background = $1 ($(outputcolor $1))"
fi
if [ -z "$clr_2" ]
then
    echo "Column 1 font color = default ($(outputcolor 2))"
else
    echo "Column 1 font color = $2 ($(outputcolor $2))"
fi
if [ -z "$clr_3" ]
then
    echo "Column 2 background = default ($(outputcolor 1))"
else
    echo "Column 2 background = $3 ($(outputcolor $3))"
fi
if [ -z "$clr_4" ]
then
    echo "Column 2 font color = default ($(outputcolor 6))"
else
    echo "Column 2 font color = $4 ($(outputcolor $4))"
fi
